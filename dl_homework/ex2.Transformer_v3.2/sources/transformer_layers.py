...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # 2. 重塑为多头形式 (N, S, E) -> (N, S, H, D) -> (N, H, S, D)
        Q = Q.view(N, S, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, S, D)
        K = K.view(N, T, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, T, D)
        V = V.view(N, T, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, T, D)
        ...
        if attn_mask is not None:
            # 将0值替换为极小值，softmax后这些位置权重≈0
            attn_scores = attn_scores.masked_fill(attn_mask == 0, -1e9)
        
        # 5. 计算注意力权重
        attn_weights = F.softmax(attn_scores, dim=-1)  # (N, H, S, T)
        attn_weights = self.attn_drop_layer(attn_weights)
        
        # 6. 加权求和
        output = torch.matmul(attn_weights, V)  # (N, H, S, D)
        
        # 7. 合并多头
        output = output.transpose(1, 2).contiguous()  # (N, S, H, D)
        output = output.view(N, S, E)  # (N, S, E)
        
        # 8. 最终投影
        output = self.proj_MLP(output)
        # ​**​*​**END OF YOUR CODE ​**​*​**​
        return output
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        # ​**​*​**START OF YOUR CODE ​**​*​**​
        # 1. 创建位置向量 [0, 1, 2, ..., max_len-1]
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)  # (max_len, 1)
        
        # 2. 计算频率因子 (embed_dim/2个不同频率)
        div_term = torch.exp(
        ...
        # 3. 应用正弦和余弦函数
        pe[0, :, 0::2] = torch.sin(position * div_term)  # 偶数位置用正弦
        pe[0, :, 1::2] = torch.cos(position * div_term)  # 奇数位置用余弦
        # ​**​*​**END OF YOUR CODE ​**​*​**​
    ...
    def forward(self, x):
        ...
        # ​**​*​**START OF YOUR CODE ​**​*​**​
        # 1. 获取前S个位置的位置编码
        pos_embed = self.pe[:, :x.size(1)]  # 动态获取序列长度
