...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # ​**​*​**START OF YOUR CODE ​**​*​**​
        # 1. 对caption进行嵌入和位置编码
        caption_embed = self.embedding_layer(captions)  # (N, T, W)
        caption_embed = self.positional_encoding_layer(caption_embed)  # (N, T, W)
        
        # 2. 准备目标序列掩码(tgt_mask)
        # 创建下三角矩阵，防止未来信息泄露
        seq_len = captions.size(1)
        tgt_mask = torch.tril(torch.ones(seq_len, seq_len)) == 0  # (T, T)
        tgt_mask = tgt_mask.to(caption_embed.device)
        
        # 3. 应用Transformer解码器
        # 使用解码器处理文本嵌入和图像特征
        decoder_output = self.transformerdecoder(caption_embed, memory, tgt_mask=tgt_mask)  # (N, T, W)
        
        # 4. 将解码器输出投影到词汇空间
        scores = self.output_layer(decoder_output)  # (N, T, V)
        # ​**​*​**END OF YOUR CODE ​**​*​**​
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        # ​**​*​**START OF YOUR CODE ​**​*​**​
        # 步骤1: 自注意力机制（带掩码）
        self_attn_output = self.self_attn(query=tgt, key=tgt, value=tgt, attn_mask=tgt_mask)
        self_attn_output = self.dropout1(self_attn_output)
        tgt = tgt + self_attn_output
        tgt = self.norm1(tgt)
        
        # 步骤2: 编码器-解码器注意力机制（交叉注意力）
        cross_attn_output = self.multihead_attn(query=tgt, key=memory, value=memory)
        cross_attn_output = self.dropout2(cross_attn_output)
        tgt = tgt + cross_attn_output
        tgt = self.norm2(tgt)
        
        # 步骤3: 前馈神经网络
        ffn_output = self.linear1(tgt)
